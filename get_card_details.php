...
if ($set_info) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE cs.sets_id = ?
        ORDER BY cs.card_id ASC
    ");
    $stmt->execute([$set_info['sets_id']]);
    $cards_in_set = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $response['cards'] = $cards_in_set;

    if (!empty($cards_in_set)) {
        $modelnum = $cards_in_set[0]['modelnum'] ?? null;
...
foreach ($response['cards'] as &$card) { // ★参照渡し(&)を使う
    ...
    if (!$response['is_set']) {
        // ★★★あなたの元のコードにあった、より安全なテキスト処理ロジックを復元★★★
        if (!function_exists('format_ability_text_single')) {
            function format_ability_text_single($raw_text) {
                if (!$raw_text) return '';
                $lines = explode("\n", $raw_text);
                $processed_lines = [];
                ...
                return str_replace(array_keys($final_replacements), array_values($final_replacements), $formatted_text);
            }
        }
        $card['text'] = format_ability_text_single($card['text'] ?? '');

        if (!function_exists('format_flavor_text_single')) {
            function format_flavor_text_single($raw_text) {
                if (!$raw_text || trim($raw_text) === '') return null;
                return nl2br(htmlspecialchars(trim($raw_text)));
            }
        }
        $modelnum = $card['modelnum'] ?? null;
        $flavor_file = $modelnum ? "flavortext/" . $modelnum . ".txt" : null;
        if ($flavor_file && file_exists($flavor_file)) {
            $card['flavortext'] = format_flavor_text_single(file_get_contents($flavor_file));
        } else {
             $card['flavortext'] = format_flavor_text_single($card['flavortext'] ?? '');
        }
    }
}
unset($card);
