...
if ($set_info) {
    ...
    $response['flavortexts'] = process_text_from_file($modelnum, 'flavortext'); // ★★★ この行を追加 ★★★
...
foreach ($response['cards'] as $key => &$card) {
    ...
    if (!$response['is_set']) {
        ...
        $flavor_file = "flavortext/" . ($card['modelnum'] ?? '') . ".txt";
        if (file_exists($flavor_file)) {
            $card['flavortext'] = format_flavor_text(file_get_contents($flavor_file));
        } else {
